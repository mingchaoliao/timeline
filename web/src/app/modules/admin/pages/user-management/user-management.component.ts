...
export class UserManagementComponent implements OnInit {
    ...
    activateOrInactivateAccount(user: User, event) {
        const isChecked = event.srcElement.checked;
        user['activeLoading'] = true;
        if (isChecked !== user.isActive) {
            this.userService.activateOrInactivateAccount(user.id, isChecked).subscribe(
                s => {
                    user.isActive = isChecked;
                    user['activeLoading'] = false;
                    NotificationEmitter.emit(Notification.success('activate/inactivate successfully'));
                },
                error => {
                    event.srcElement.checked = user.isActive;
                    user['activeLoading'] = false;
                    NotificationEmitter.emit(Notification.error(error.error.message, `Unable to activate/inactivate user "${user.name}"`));
                }
            );
        }
    }
